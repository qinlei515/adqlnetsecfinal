...
public class KSAuthenticate 
{
	...
	public static SecretKey authenticate(DataOutputStream toServer, DataInputStream fromServer, DHParameterSpec ourSpecs)
	{
		try 
		{
			...
			toServer.write(pubKey.getEncoded());
			// TODO: For simplicity, we currently assume we will receive two challenge from the server.
			toServer.write(Common.handleChallenge1(fromServer));
			toServer.write(Common.handleChallenge2(fromServer));
			// Challenges are done. Re-send original request.
			toServer.write(pubKey.getEncoded());
			byte[] resp = Common.getResponse(fromServer);
			if(BufferUtils.equals(resp, Constants.SERVER_KEY_RESET))
			{
				//TODO: Update the server's primary and secondary public keys.
				//And re-send the request yet again.
			}			
